...
12. **Trực quan tỉ lệ sống sót theo giới tính**  (Countplot)
   - Vẽ `countplot` với `x='Survived', hue='Sex'` để so sánh số người sống/chết theo giới tính.

13. **Trực quan tỉ lệ sống sót theo hạng vé (Pclass)**  (Countplot)
   - Vẽ `countplot` với `x='Survived', hue='Pclass'` để so sánh số người sống/chết theo hạng vé.

14. **Trực quan tỉ lệ sống sót theo giới tính và thang đo tuổi tác**  (Violinplot)
   - Vẽ `violinplot` với `x='Sex', y='Age', hue='Survived'` để quan sát phân bố tuổi sống sót/thiệt mạng cho từng giới tính.

15. **Trực quan xác suất sống sót theo nhóm đi cùng (familySize)**  (Histogram)
   - Vẽ `histplot` với `x='familySize', hue='Survived'` để xem nhóm đi 1 mình/đông người ảnh hưởng thế nào đến tỉ lệ sống sót.

16. **Trực quan xác suất sống sót theo giá vé (Fare)**  (Histogram)
   - Vẽ `histplot` với `x='Fare', hue='Survived'` để quan sát mối liên hệ giữa mức giá vé và khả năng sống sót.

17. **Trực quan sống sót theo hạng vé (Pclass) và cảng lên tàu (Embarked)**  (Pointplot)
   - Vẽ `catplot` kiểu `point` với `x='Pclass', y='Survived', hue='Sex'` để thể hiện tỉ lệ sống sót theo từng hạng vé, tách theo giới tính (kết hợp thêm thông tin cảng trong dữ liệu).
